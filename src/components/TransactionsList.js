...
function TransactionsList({ transaction, search, setTransactionsList }) {
  ...
  listItems = filteredTransactions.map(item => {
    return <Transaction key={item.id} transactionItem={item} setTransactionsList={setTransactionsList} />;
  });
    ...
    <table className="ui celled striped padded table">
      <tbody>
        <tr>
          ...
          <th style={{ backgroundColor: "rgba(0, 0, 0, 0.10)", cursor: "pointer" }}>
            <h3 className="ui center aligned header" onClick={sortTransactionList} title="click to sort by description">
              Description
            </h3>
          </th>
          <th style={{ backgroundColor: "rgba(0, 0, 0, 0.10)", cursor: "pointer" }}>
            <h3 className="ui center aligned header" onClick={sortTransactionList} title="click to sort by category">
              Category
            </h3>
          </th>
          <th style={{ backgroundColor: "rgba(0, 0, 0, 0.10)", cursor: "pointer" }}>
